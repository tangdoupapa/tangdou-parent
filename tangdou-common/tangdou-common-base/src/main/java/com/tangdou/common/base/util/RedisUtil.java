...
public class RedisUtil {
     ...
     * 删除用户Token
     *
     * @param tokens 待删除的Token
     * @return
     */
    public static boolean removeToken(String... tokens) {
        try {
            if (ArrayUtil.isNotEmpty(tokens)) {
                String[] tokenArrays = Arrays.stream(tokens).map(x -> StrUtil.format("prefix_user_token_{}", x)).toArray(String[]::new);
                remove(tokenArrays);
                return true;
            }
            return true;
        } catch (Exception e) {
            log.error("用户Token删除失败", e);
            return false;
        }
    }
    ...
    public static <T> T get(String key, Class<T> clazz) {
        Object o = get(key);
        return ObjectUtil.isEmpty(o) ? null : ReflectUtil.newInstance(clazz, o);
    }
